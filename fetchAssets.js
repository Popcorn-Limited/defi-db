...
async function getBaseTokens() {
  const { data } = await axios.get(
    "https://enso-scrape.s3.us-east-2.amazonaws.com/output/backend/baseTokens.json"
  );
  return data.map((t) => {
    return {
...
async function getDefiTokens() {
  const { data } = await axios.get(
    "https://enso-scrape.s3.us-east-2.amazonaws.com/output/backend/defiTokens.json"
  );
  return data.map((t) => {
    return {
      address: getAddress(t.token.address),
      name: t.token.name,
      symbol: t.token.symbol,
      decimals: t.token.decimals,
      logoURI: t.protocol.logo,
      chainId: t.token.chain,
    };
  });
}
...
async function getExistingAddresses(chainId) {
  const { data } = await axios.get(
    `https://raw.githubusercontent.com/Popcorn-Limited/defi-db/main/archive/assets/addresses/${chainId}.json`
  );
  ...
  return addresses.map((address, i) => {
    ...
    if (
      ...
      return {
        ...
        symbol: results[i + 1].result,
        decimals: results[i + 2].result,
...
async function getNewAssets(yieldOptions, chainId, ensoAssets) {
  console.log("fetching assets for: ", chainId);
  const result = {};
  const existingAddresses = await getExistingAddresses(chainId);

  // Check for duplicate assets in enso
  const filteredEnsoAssets = ensoAssets
    .filter((asset) => asset.chainId === chainId)
    .filter((asset) => !existingAddresses.includes(asset.address));
  ...
  const availableLpProtocols = availableProtocols
    .map((p) => p.key)
    .filter((key) => lpProtocols.includes(key));

  // Fetch lp addresses
  let filteredLpTokens = [];
  if (availableLpProtocols.length > 0) {
    const lpTokens = await Promise.all(
      availableLpProtocols.map(async (protocol) => {
        const protocolAssets = await yieldOptions.getProtocolAssets({
          chainId,
          protocol,
        });
        return protocolAssets.map((a) => {
          return { address: a, protocol: protocol };
        });
      })
    );

    // Check for duplicate assets in lp addresses
    filteredLpTokens = lpTokens
      .filter((lp) => !existingAddresses.includes(lp.address))
      .flat();
  ...
  if (filteredLpTokens.length > 0 || filteredAddresses.length > 0) {
    ...
    if (filteredLpTokens.length > 0) {
      const lps = await getLpAssets(
        filteredLpTokens.map((lp) => lp.address),
        chainId,
        filteredLpTokens[0].protocol,
        client
      );
    ...
    if (filteredAddresses.length > 0) {
      ...
      tokens.forEach((asset) => {
        if (asset) result[asset.address] = asset;
      });
    }
  }
...
async function main() {
  ...
  chains.forEach(async (chainId) => {
    const { tokens, addresses } = await getNewAssets(yieldOptions, chainId, [
      ...baseTokens,
      ...defiTokens,
    ]);
    writeFileSync(
      `./archive/assets/addresses/${chainId}.json`,
      JSON.stringify(addresses),
      "utf-8"
    );
    if (Object.keys(tokens).length > 0) {
      const data = readFileSync(
        `./archive/assets/tokens/${chainId}.json`,
        "utf8"
      );
