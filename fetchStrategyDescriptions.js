...
function getAaveV2Metadata(name) {
  return {
    name: "Aave",
    description: addGenericStrategyDescription("lending", "AaveV2"),
    resolver: "aaveV2",
  };
}
...
function getAaveV3Metadata(name) {
  return {
    name: "Aave",
    description: addGenericStrategyDescription("lending", "AaveV3"),
    resolver: "aaveV3",
  };
}
...
function getCompoundV2Metadata(name) {
  return {
    name: "Compound",
    description: addGenericStrategyDescription("lending", "CompoundV2"),
    resolver: "compoundV2",
  };
}
...
function getCompoundV3Metadata(name) {
  return {
    name: "Compound",
    description: addGenericStrategyDescription("lending", "CompoundV3"),
    resolver: "compoundV3",
  };
}
...
function getFluxMetadata(name) {
  return {
    name: "Flux",
    description: addGenericStrategyDescription("lending", "Flux"),
    resolver: "flux",
  };
}
...
function getBeefyMetadata(name) {
  return {
    name: "Beefy",
    description: addGenericStrategyDescription(
      "automatedAssetStrategy",
      "Beefy"
    ),
    resolver: "beefy",
  };
...
const EXCEPTIONS = {
  "0xE3267A9Ff2d38B748B6aA202e006F7d94Ca22df3": {
    name: "Sommelier",
    description:
      "**Automated Asset Strategy** - The vault supplies assets into Sommelier to earn yield on their automated asset strategies.",
    resolver: "sommelier",
  },
};
...
async function getStuffByChain(chainId) {
  ...
  if (filteredStrategies.length > 0) {
    ...
    filteredStrategies.forEach((address, i) => {
      if (i > 0) i = i * 2;
      const metadata = getFactoryMetadata({ address, name: res[i] });
...
async function main() {
  for (let i = 0; i < chains.length; i++) {
    ...
    writeFileSync(
      `./archive/descriptions/strategies/${chains[i]}.json`,
      JSON.stringify(data),
      "utf-8"
    );
  }
